...
class MovieRepository
  ...
  def save_json
    saved_data = @movies.map do |movie|
      {id: movie.id, name: movie.name, rating: movie.rating, year: movie.year, description: movie.description}
    end
  ...
  def load_json
    ...
    movies = JSON.parse(searlized_movies, {symbolize_name: true})
    movies.each do |movie|
      @movies << Movie.new(movie)
    end
    @next_id = @movies.last.id
  end
end
